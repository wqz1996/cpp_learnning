...
if __name__ == '__main__':
    ...
    ## multipule frequency signal
    velocity_signal_1 = amplitude * 0.25 * np.sin(2 * np.pi * frequency * t +
                                                  phase_bias) + amplitude_bias
    velocity_signal_2 = amplitude * 0.25 * np.sin(2 * np.pi * 0.15 * t +
                                                  phase_bias) + amplitude_bias
    velocity_signal_3 = amplitude * 0.25 * np.sin(2 * np.pi * 0.2 * t +
                                                  phase_bias) + amplitude_bias
    velocity_signal_4 = amplitude * 0.25 * np.sin(2 * np.pi * 0.25 * t +
                                                  phase_bias) + amplitude_bias
    velocity_signal = [
        a + b + c + d
        for a, b, c, d in zip(velocity_signal_1, velocity_signal_2,
                              velocity_signal_3, velocity_signal_4)
    ]
    acceleration_signal = []
    for idx in range(len(velocity_signal)):
        if idx is 0:
            acceleration_signal.append(0)
        else:
            acceleration_signal.append(
                (velocity_signal[idx] - velocity_signal[idx - 1]) * fs)
    ###################################################################
    ...
    for v, a in zip(velocity_signal_with_white_noise,
        ...
        K_0.append(kalman_filter._K[0, 0])
        K_1.append(kalman_filter._K[1, 0])
    ...
    for v, a in zip(velocity_signal_with_white_noise,
        ...
        fuse_K_0.append(fuse_acc_kalman_filter._kalman_filter._K[0, 0])
        fuse_K_1.append(fuse_acc_kalman_filter._kalman_filter._K[1, 0])
    fuse_acc_a_error = [
    ...
    plt.plot(t, K_0, label='K_0')
    plt.plot(t, K_1, label='K_1')
    plt.plot(t, fuse_K_0, label='fuse_K_0')
    plt.plot(t, fuse_K_1, label='fuse_K_1')
    ...
    plt.subplot(3, 1, 3)
    plt.plot(t, v_white_noise, label='v_white_noise')
    plt.plot(t, a_white_noise, label='a_white_noise')
    plt.title(
