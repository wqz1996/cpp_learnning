...
def compute_phase_difference(signal1, signal2):
    ...
    peak_index = np.argmax(phase_diff)

    delay = peak_index / len(signal1)
...
def align_signals(signal1, signal2):
    ...
    for delay in range(len(signal1)):
...
if __name__ == '__main__':
    ...
    ##############################################
    delay_from_phase_difference = compute_phase_difference(
        interested_origin_signal, interested_delay_signal)

    fix_phase_index = round(delay_from_phase_difference * fs)
    print("Phase different delay: {}".format(delay_from_phase_difference))

    fix_signal_by_phase_difference = delay_signal[
        interested_start_t * fs + fix_phase_index:interested_end_t * fs +
        fix_phase_index]

    delay_from_mutual_information = align_signals(interested_origin_signal,
                                                  interested_delay_signal)
    print("Mutual Information delay: {}".format(delay_from_mutual_information))
    ...
    plt.plot(interested_t,
             fix_signal_by_phase_difference,
             label='fixed_by_phase')
    plt.title("Signal")
    plt.xlabel('Time/s')
