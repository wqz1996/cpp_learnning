...
public:
  IIR(const vector<double> &numerators, const vector<double> &denominators)
  ...
  double Filter(double input) {
    ...
    for (int i = 0; i < _numerators.size(); i++) {
      output += _numerators[i] * _x[i];
    }

    _y.pop_back();
    for (int i = 1; i < _denominators.size(); i++) {
      output -= _denominators[i] * _y[i - 1];
    }
    if (_denominators.front() > 1e-15) {
      output /=
          _denominators
              .front(); //通常反馈系数第一项是1，但是通用形式的并不一定是1，这里面保证了普适性
    }
    _y.push_front(output);
    return output;
  }
};
