...
class VErrorAnalysis:
    def __init__(self, session, config_dict: dict) -> None:
        ...
        start_ts = datetime.strptime(config_dict.get("start_time"), "%Y-%m-%d %H:%M:%S.%f").timestamp()
        end_ts = datetime.strptime(config_dict.get("end_time"), "%Y-%m-%d %H:%M:%S.%f").timestamp()
    ...
    def get_query(self):
        for index, _ in enumerate(range(len(self._search_ts_list) - 1)):
            print(self._vehicle_name, self._search_ts_list[index], self._search_ts_list[index + 1])
            v_error_list = []
            try:
                query_msg = self._session.execute(self._control_cmd_sql.format(self._vehicle_name,
                                                                        self._search_ts_list[index],
            ...
            print(sum(v_error_list), len(v_error_list))
            print(self.v_error_statistic)
...
if __name__ == '__main__':
    ...
    config_dict = {
        ...
        "start_time": '2023-03-31 00:00:00.00',
        "end_time": '2023-04-01 00:00:00.00',
        "time_step": 80
    }
    v_error = VErrorAnalysis(session=session, config_dict=config_dict)
    v_error.get_query()
    print(v_error.v_error_statistic)
